...
Try it out by downloading some material we'll use later on (a
[wordlist][wordlist] from John Lawler at University of Michigan) from the
following url:
...
> Use one of the above commands to save the file as `wordlist.txt`.
>
> > ## `wget` and `curl` Commands
> >
> > ```
> > {{ site.remote.prompt }} wget -0 wordlist.txt https://websites.umich.edu/~jlawler/wordlist
> > # or
> > {{ site.remote.prompt }} curl -o wordlist.txt -L https://websites.umich.edu/~jlawler/wordlist
> > ```
> > {: .language-bash}
...
The ondemand "Files" tab provides a graphical interface to all your files on
Borah.

{% include figure.html max-width="75%" caption=""
   file="/fig/ood-dash-files.png"
   alt="Ondemand dashboard highlighting the Files app" %}
...
use `scp` ("secure copy"). The syntax can be a little complex for new users,
but we'll break it down. The `scp` command is a relative of the `ssh` command
...
```
{{ site.local.prompt }} scp myfile {{ site.remote.user }}@{{ site.remote.login }}:
```
{: .language-bash}
...
To view the contents of a tarfile, _without_ unpacking the file, we can use the
`-t` flag.
...
```
{{ site.remote.prompt }} mv hpc-carpentry-amdahl-46c9b4b amdahl
```
{: .language-bash}
